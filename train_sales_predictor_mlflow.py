...
def train(
    csv_path: str, out_model_path: str = "sales_value_model.keras", lags: int = 7
):
    ...
    print(
        f"Previsão do valor de vendas para o próximo dia após {dates_all[-1]}: R$ {next_value_pred:,.2f}"
    )
...
def main():
    ...
    parser.add_argument(
        "--csv_path",
        type=str,
        default="sales_dataset.csv",
        help="Caminho para o CSV de dados de vendas",
    )
    parser.add_argument(
        "--out_model",
        type=str,
        default="sales_value_model.keras",
